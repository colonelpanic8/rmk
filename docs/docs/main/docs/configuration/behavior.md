...
- `opposite_hand_hold`: Keeps a tap-hold undecided after `hold_timeout` and activates its hold action only when a key tagged as the opposite hand, or as bilateral, is pressed. A same-hand ordinary key resolves it as a tap even after the timeout. A second same-hand tap-hold whose hold action is a modifier remains pending, allowing a later opposite-hand key to activate both modifiers as a chord; tapping that second key resolves the pending keys as taps. This and `unilateral_tap = true` are mutually exclusive.
