...
[Rynk](../features/rynk) is RMK's native host protocol — an alternative to Vial — and it is now enabled by default. The `rmk` crate's default features changed:

| v0.8 default features                               | v0.9 default features                  |
| --------------------------------------------------- | -------------------------------------- |
| `defmt`, `storage`, `vial`, `vial_lock`, `watchdog` | `defmt`, `storage`, `rynk`, `watchdog` |

Rynk and Vial are mutually exclusive — enable exactly one. Note that `vial_lock` is also no longer a default feature; add it back explicitly if you use Vial's unlock keys.
...
**Option A — switch to Rynk (recommended).** The default features already include `rynk`, so you only need to update `[host]`:
...
**Option B — keep Vial.** Turn off default features and re-enable `vial` (plus `vial_lock` if you use unlock keys), and keep `vial_enabled = true` (the default) in `keyboard.toml`:
...
**Step 1 — pick the protocol in `Cargo.toml`.** Rust projects list features explicitly, so if you already listed `vial`, nothing forces a change. To move to Rynk, replace `vial` (and `vial_lock`) with `rynk` in your feature list.
