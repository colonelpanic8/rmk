...
v0.9 introduces [Rynk](../features/rynk), RMK's optional native host protocol, while keeping Vial as
the default. v0.9 also updates the embassy and BLE dependency stacks, unifies the event/processor
Rust API, restructures `keyboard.toml` to separate the electrical matrix, physical layout, and
logical keymap, and includes a few smaller breaking changes.
...
| ----------------------------------------------------------------------------------------- | :-------------------: | :-----------------: |
| [`keyboard.toml` now drives Cargo features](#keyboardtoml-now-drives-cargo-features)      |          ✅           |          —          |
...
### `keyboard.toml` now drives Cargo features

**Who this affects:** everyone using `keyboard.toml`. Rust API users keep configuring RMK through
Cargo features exactly as before.

Capabilities are now activated by `keyboard.toml` itself. Your `Cargo.toml` only needs the features
that select _dependencies_ — logging (`defmt`/`log`), the chip BLE feature (`nrf52840_ble`, …),
`rp2040`, the DFU flavor (`dfu`/`dfu_rp`/`dfu_nrf`), the display driver family (`ssd1306`/
`oled_async`), and `adafruit_bl`. Everything else (`storage`, `vial`, `rynk`, `split`,
`async_matrix`, `watchdog`, `steno`, `host_lock`, `dfu_lock`, `passkey_entry`) is derived from the
TOML, and RMK stops the build with a fix-it error when the two disagree.

**Before (v0.8):**

```toml title="Cargo.toml"
rmk = { version = "0.8", default-features = false, features = [
    "defmt", "storage", "vial", "watchdog", "async_matrix", "nrf52840_ble", "split",
] }
```
...
```toml title="Cargo.toml"
rmk = { version = "0.9", default-features = false, features = ["defmt", "nrf52840_ble"] }
```

Capabilities that previously existed only as Cargo features gained `keyboard.toml` fields:

| Capability            | New field                              | Default |
| --------------------- | -------------------------------------- | ------- |
| Async matrix scanning | `[keyboard]` `async_matrix = true`     | `false` |
| Hardware watchdog     | `[keyboard]` `watchdog = false`        | `true`  |
| Plover HID steno      | `[keyboard]` `steno = true`            | `false` |
| Bootloader jump key   | `[keyboard]` `bootloader = "adafruit"` | unset   |
| DFU firmware update   | `[dfu]` `enabled = true`               | `false` |

Notes:

- If your feature list had `async_matrix`, add `async_matrix = true` to `[keyboard]` (or drop the
  feature to scan synchronously).
- **The watchdog now defaults to on for `keyboard.toml` users.** If you previously disabled it by
  omitting the `watchdog` feature, set `watchdog = false` in `[keyboard]`.
- The DFU lock no longer needs the `dfu_lock` feature — it enables automatically when
  `[dfu].unlock_keys` is set.
- `[dfu]` needs the new `enabled = true` in addition to the `dfu_rp`/`dfu_nrf` Cargo feature (chip
  default configs ship a `[dfu]` partition layout, so section presence can't mean activation).
...
remains the default host protocol. The `rmk` crate's default features changed only to replace the
removed `vial_lock` alias with its underlying `host_lock` feature:

| v0.8 default features                               | v0.9 default features                               |
| --------------------------------------------------- | --------------------------------------------------- |
| `defmt`, `storage`, `vial`, `vial_lock`, `watchdog` | `defmt`, `storage`, `vial`, `host_lock`, `watchdog` |
...
Then turn off default features so the default `vial` feature doesn't conflict — `rynk` itself is
activated by the `[host]` section above:
...
**Step 1 — pick the protocol in `Cargo.toml`.** Rust projects that list `vial` explicitly do not need
to change it. To move to Rynk, disable default features and replace `vial` (and `vial_lock`) with
`rynk` in your explicit feature list.
