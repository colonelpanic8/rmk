...
This step applies only when you build with the [Vial](https://get.vial.today/) protocol (the `vial` feature). RMK supports the [vial app](https://get.vial.today/), an open-source cross-platform(windows/macos/linux/web) keyboard configurator. So the vial like keymap definition has to be imported to the firmware project.
...
If you use the default [Rynk](../../features/rynk) protocol instead of Vial, you don't need a `vial.json` — skip this step.
