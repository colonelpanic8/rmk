...
::: warning Vial and [Rynk](./rynk) are mutually exclusive firmware protocols. Vial is enabled by the `rmk` crate's default Cargo features; enable Rynk explicitly only after disabling those defaults. :::
...
In `Cargo.toml`, keep RMK's default features and add the features your keyboard needs:

```toml title="Cargo.toml"
rmk = { version = "...", features = ["rp2040"] }
```

Add any chip, BLE, display, or other dependency features your board needs — capabilities like split and storage activate from `keyboard.toml`.
