...
::: info There are many examples for split keyboards in [RMK examples folder](https://github.com/HaoboGu/rmk/tree/main/examples/use_config). :::
...
::: details RMK uses `embedded-io-async` as the abstract layer of wired communication. Any device that implements `embedded-io-async::Read` and `embedded-io-async::Write` traits can be used as RMK split central/peripheral. That unlocks many possibilities of RMK's split keyboard.
...
::: tip [storage](./storage.md) feature is required for BLE split. :::
...
Split support is activated by the `[split]` section in `keyboard.toml` (the `split` Cargo feature is only needed with the pure Rust API). Add the `[[bin]]` section for both central and peripheral in `Cargo.toml`:
