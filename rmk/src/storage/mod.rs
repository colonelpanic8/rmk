...
pub(crate) async fn read_layer_metadata(layer: u8) -> Option<LayerMetadata> {
    ...
    FLASH_CHANNEL.send(FlashOperationMessage::ReadLayerMetadata(layer)).await;
        ...
        loop {
            ...
            let write_result: Result<(), SSError<F::Error>> = match info {
                ...
                FlashOperationMessage::LayerMetadata { layer, metadata } => {
                    self.store_data(
                        StorageKey::LayerMetadata(layer),
                        &StorageData::LayerMetadata(metadata),
                    )
                    .await
                }
...
mod tests {
    ...
    fn layer_metadata_survives_flash_map_reopen() {
        block_on(async {
            ...
            let mut reopened =
                MapStorage::<StorageKey, _, _>::new(flash, MapConfig::new(range), Cache::new_uncached());
            let stored = reopened
