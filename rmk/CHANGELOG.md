...
- Add auto mouse layer behavior: automatically activate a configured layer when X/Y cursor motion from a pointing device is detected, and deactivate it after a `timeout` of inactivity ([#781](https://github.com/HaoboGu/rmk/issues/781))
- Add Vial key override support, backed by forks: the Key Override tab shows `fork_max_num` editable entries (default 8) which take effect immediately and persist to flash. Every GUI-configurable field and option is honored: trigger/negative/suppressed modifier masks, QMK's AND matching with left/right pairing (or OR matching via the "one mod" option), the per-layer mask (tested against the key's source layer, QMK parity), the per-entry enable toggle, and all activation/deactivation option bits — overrides can activate mid-hold when a required modifier goes down or a negative modifier is released, deactivate when the state stops matching or another key goes down, and honor `no_reregister_trigger`/`no_unregister_on_other_key_down`. `keyboard.toml` forks keep the legacy latched decision (their options default to trigger-down activation only, no deactivation on other keys)

### Changed

- **BREAKING**: the `Fork` struct (Rust API) replaces `kept_modifiers` with its inverse, `suppressed_modifiers`, and gains two fields: `layers` (optional layer mask) and `options` (a single byte mirroring Vial's key-override options: matching mode, activation events, per-entry enable, no_reregister/no_unregister). RMK's native behavior stays the default and `keyboard.toml` is unaffected: `kept_modifiers` remains the config key, translated at build time, and no new TOML options are introduced
