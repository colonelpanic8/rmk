...
- Add runtime-configurable per-layer lighting scenes over Rynk: hosts can store per-layer, per-LED effects on the keyboard (`GetLightingSceneStatus`/`GetLightingScenes`, `SetLightingSceneCell`/`UnsetLightingSceneCell`, an atomic `BeginLightingSceneReplace`/`PutLightingSceneChunk`/`CommitLightingSceneReplace`/`AbortLightingSceneReplace` transaction, and `SetLightingLayerPolicy`). Scene cells compose in the layer band — above the board's static layer scenes, below the transient TTL overlay — and persist via RMK storage, loading on boot and clearing with a storage reset. Boards opt in by choosing a scene capacity on the standard lighting engine and advertising it with `RynkLightingController::with_scene_capacity`
