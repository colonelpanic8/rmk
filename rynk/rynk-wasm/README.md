...
`rynk-ble`. `events_dropped()` reports topic queue overflow. Client operations
are cancellation-safe because the independently running reader retains the
transport and its frame boundary.
